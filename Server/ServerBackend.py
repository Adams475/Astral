...
class ServerInstance:
    ...
    def decrypt_keys(self, password):
        ...
        try:
            self.rsa_private_key_enc_dec = Astral.utils.decrypt_rsa_private_key(encrypted_rsa_key_file_enc_dec,
                                                                                password)
            self.rsa_private_key_signing = Astral.utils.decrypt_rsa_private_key(encrypted_rsa_key_file_signing,
                                                                                password)
    ...
    def parse_raw_data(self, data):
        ...
        if REQUEST_HEADERS[header_byte] == "RSA":
            return Astral.utils.decrypt_message(raw_data, self.rsa_private_key_enc_dec)
    ...
    def handle_request(self, message):
        ...
        hash_msg = Astral.utils.hash_message(body.encode('latin-1'))
        ...
        if verb == 'Enroll':
            if self.enroll_user(body):
                ...
                return json.dumps(response).encode('latin-1'), False  # Repetitive code
            else:
                response = json.dumps({"Status": "Fail", "Reason": "Already Enrolled", "Signature": signed_msg})
                return json.dumps(response).encode('latin-1'), False
        elif verb == 'Login':  # Login is a persistent connection
            challenge = Astral.utils.generate_128_bit_random_number()
            if body in self.client_passwords:
                self.hmac_randoms[body] = challenge
                response = json.dumps({"Status": "Success", "Challenge": challenge.decode('latin-1'), "Signature": signed_msg})
                return json.dumps(response).encode('latin-1'), True
            else:
            ...
            if user not in self.hmac_randoms:
                response = json.dumps({"Status": "Fail", "Reason": "User Not Logged In",
                                       "Signature": signed_msg})
                return json.dumps(response).encode('latin-1'), False
            if Astral.utils.make_hmac(self.client_passwords[user], self.hmac_randoms[user]) != hmac.encode("latin-1"):
                response = json.dumps({"Status": "Fail", "Reason": "HMAC Does Not Match",
                                       "Signature": signed_msg})
                return json.dumps(response).encode('latin-1'), False

            session_key_enc_dec = Astral.utils.generate_128_bit_random_number()
            session_key_signing = Astral.utils.generate_128_bit_random_number()

            # TODO Encrypt keys, send them in a JSON, Store the session keys for this client

            response = json.dumps({"Status": "Success", "Signature": signed_msg})
            return json.dumps(response).encode('latin-1'), False  # needs to be true in the future

        response = json.dumps({"Status": "Bad Verb", "Signature": signed_msg})
        return json.dumps(response).encode(), False
    ...
    def sign_message(self, hash_msg):
        return Astral.utils.sign_message(hash_msg, self.rsa_private_key_signing)
